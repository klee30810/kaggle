...
def train_predict(train_file, test_file, feature_map_file, predict_valid_file, predict_test_file,
                  feature_imp_file, n_est=100, n_leaf=200, lrate=.1, n_min=8, subcol=.3, subrow=.8,
                  subrow_freq=100, n_stop=100):

    model_name = os.path.splitext(os.path.splitext(
        os.path.basename(predict_test_file))[0])[0]
    ...
    for i, (i_trn, i_val) in enumerate(cv.split(X, y), 1):
        ...
        clf = lgb.train(params, trn_lgb, n_est, val_lgb,
                        early_stopping_rounds=n_stop, verbose_eval=100)
    ...
    np.savetxt(predict_test_file, p, fmt='%.6f', delimiter=',')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--predict-valid-file',
                        required=True, dest='predict_valid_file')
    parser.add_argument('--predict-test-file',
                        required=True, dest='predict_test_file')
    parser.add_argument('--feature-imp-file', required=True,
                        dest='feature_imp_file')
    ...
    parser.add_argument('--subrow-freq', type=int,
                        default=100, dest='subrow_freq')
